<?php
/**
 * Plugin Name:       Psych Advanced Mission Engine (Refactored)
 * Description:       موتور مرکزی برای مدیریت ماموریت‌ها و فعالیت‌های تعاملی با معماری ماژولار.
 * Version:           10.0.0
...
/**
 * Main Class for the Advanced Mission Engine.
 * This refactored version improves structure, stability, and frontend integration.
 */
final class Psych_Advanced_Mission_Engine_Core {
    ...
    /** @var string The name of the custom database table for results. */
    private $db_table_name;

    /** @var array Holds the class names of dynamically loaded activity types. */
    ...
    /**
     * Registers all WordPress hooks for the engine.
     */
    ...
    private function load_and_register_activities() {
        ...
        if (!file_exists($activity_path . 'interface-psych-activity.php')) return;
    ...
    // =============================================================================
    // SECTION: Shortcode Rendering
    // =============================================================================

    /**
     * Main [psych_mission] shortcode handler.
     * Acts as a controller to check permissions and dispatch to the correct activity class.
     */
    public function render_mission_shortcode($atts, $content = null) {
        ...
        // Determine the target user (the one the mission is for)
        ...
        // Check if the mission submission capacity is full
        $required_count = intval($atts['required_actors']);
        if ($required_count > 0 && $this->get_submission_count($target_user_id, $atts['id']) >= $required_count) {
            return '<div class="psych-notice-info">ظرفیت پاسخ‌دهی برای این ماموریت تکمیل شده است.</div>';
        }

        // Check if the current user (actor) has permission to perform this mission
        ...
        if (!$permission_check['allowed']) {
            return '<div class="psych-error">' . esc_html($permission_check['message']) . '</div>';
        }

        // If the actor is the target, check if they've already completed it
        ...
        // Instantiate and render the specific activity type
    ...
    public function render_mission_link_shortcode($atts) {
        ...
        if (!get_current_user_id() || empty($atts['url'])) return '';
        $link = add_query_arg('target_user_id', get_current_user_id(), esc_url($atts['url']));
        return '<div class="psych-share-link-container"><input type="text" readonly value="' . $link . '" /><button class="psych-copy-btn">کپی</button></div>';
    }

    public function render_mission_progress_shortcode($atts) {
        $atts = shortcode_atts(['id' => '', 'required' => 1, 'text' => 'تعداد پاسخ: %d از %d'], $atts);
        if (empty($atts['id']) || !get_current_user_id()) return '';
        $current_count = $this->get_submission_count(get_current_user_id(), $atts['id']);
        return '<div class="psych-mission-progress-counter">' . sprintf(esc_html($atts['text']), $current_count, intval($atts['required'])) . '</div>';
    }

    // =============================================================================
    // SECTION: AJAX and Mission Completion Logic
    // =============================================================================
    ...
    public function ajax_complete_activity() {
        ...
        $target_user_id = intval($payload['target_user_id']);
        $activity_id = sanitize_text_field($payload['activity_id']);
        $flag_to_set = sanitize_key($payload['flag']);

        if (!$target_user_id) wp_send_json_error(['message' => 'کاربر هدف نامعتبر.']);

        $actor_user_id = get_current_user_id();
        $status = ($payload['needs_approval'] === 'true') ? 'pending_approval' : 'completed';
        ...
        $wpdb->insert($this->db_table_name, [
            ...
            'activity_type' => sanitize_key($payload['activity_type']),
            'activity_id' => $activity_id,
            'response_data' => wp_json_encode($payload['response_data'] ?? []),
            'status' => $status,
            'submitted_at' => current_time('mysql', 1),
        ]);

        // Check if the mission is now fully complete
        $required_actors = intval($payload['required_actors'] ?? 1);
        $current_count = $this->get_submission_count($target_user_id, $activity_id);

        if ($current_count >= $required_actors) {
            if ($status === 'completed') {
                $this->finalize_mission($target_user_id, $flag_to_set, $payload['rewards'], $activity_id);
            } else {
                // Notify coach for approval if needed
        ...
        wp_send_json_success([
            ...
            'status' => $status,
            'flag_set' => $flag_to_set, // Pass the flag back to the frontend
            'progress' => ['current' => $current_count, 'required' => $required_actors]
        ]);
    }

    public function ajax_coach_approve_mission() {
        // Placeholder for coach approval logic.
        wp_send_json_error(['message' => 'هنوز پیاده‌سازی نشده.']);
    }

    private function finalize_mission($user_id, $flag, $rewards_str, $activity_id) {
        // This is the key integration point with Path Engine.
        if (!empty($flag) && function_exists('psych_complete_mission_by_flag')) {
            psych_complete_mission_by_flag($flag, $user_id);
        }

        // Award points, badges, etc.
    ...
    // =============================================================================
    // SECTION: Helper & Database Functions
    // =============================================================================
    ...
    private function can_actor_perform_mission($allowed_actors, $target_user_id, $actor_user_id) {
        ...
        if (function_exists('psych_get_user_coach_id') && $actor_user_id == psych_get_user_coach_id($target_user_id) && in_array('coach', $allowed)) return ['allowed' => true];
    ...
    public function inject_assets() {
        if (!apply_filters('psych_should_inject_assets_final_core', false) || empty($GLOBALS['psych_loaded_activities'])) return;

        $css = $this->get_shared_css();
        $js = $this->get_shared_js();

        foreach ($GLOBALS['psych_loaded_activities'] as $activity) {
            if (method_exists($activity, 'get_css')) $css .= $activity->get_css();
            if (method_exists($activity, 'get_javascript')) $js .= $activity->get_javascript();
        }

        $js_vars = ['ajax_url' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('psych_activity_nonce')];

        if (!empty($css)) echo '<style id="psych-activities-styles">' . $css . '</style>';
        if (!empty($js)) {
            echo '<script id="psych-activities-vars">window.psychActivities = ' . json_encode($js_vars) . ';</script>';
            echo '<script id="psych-activities-script">(function($){' . $js . '})(jQuery);</script>';
        }

        $GLOBALS['psych_loaded_activities'] = []; // Clear after injection
    }

    private function get_shared_css() {
        return ".psych-activity-container{background:#fff;border:1px solid #e9ecef;border-radius:8px;padding:25px;margin:20px 0;box-shadow:0 2px 8px rgba(0,0,0,0.05)}.psych-activity-submit-btn{background:#28a745;color:#fff;border:none;padding:12px 25px;border-radius:6px;cursor:pointer;}.psych-activity-feedback{margin:15px 0;padding:15px;border-radius:6px;border-left:5px solid}.psych-activity-feedback.success{background:#d4edda;color:#155724;border-color:#28a745}.psych-activity-feedback.error{background:#f8d7da;color:#721c24;border-color:#dc3545}";
    }

    private function get_shared_js() {
        // REFACTORED: Removed location.reload() and added a custom event dispatch.
        return "
        window.psychActivities.submitActivity = function(container, responseData) {
            ...
            const payload = {
                ...
                flag: config.setsFlagOnComplete,
            ...
            .done(function(response) {
                if (response.success) {
                    ...
                    const eventDetail = {
                        ...
                        flag: response.data.flag_set
                    };
                    const missionEvent = new CustomEvent('psych:missionCompleted', { detail: eventDetail });
                    document.dispatchEvent(missionEvent);

                    // Show success feedback to the user
                    container.find('.psych-activity-feedback').removeClass('error').addClass('success').html(response.data.message).slideDown();
                    btn.text('ارسال شد'); // Keep button disabled and text changed
                } else {
                    container.find('.psych-activity-feedback').removeClass('success').addClass('error').html(response.data.message || 'خطا در ثبت.').slideDown();
                    btn.prop('disabled', false).html(originalText);
                }
            })
            .fail(function() {
                container.find('.psych-activity-feedback').removeClass('success').addClass('error').html('خطای سرور.').slideDown();
                btn.prop('disabled', false).html(originalText);
            });
        };
        ";
