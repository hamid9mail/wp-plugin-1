<?php
/**
 * Plugin Name:       PsychoCourse Path Engine (v4 - FINAL SPA Edition)
...
// =============================================================================
if (!function_exists('psych_path_get_viewing_context_v4')) {
    function psych_path_get_viewing_context_v4() { return PsychoCourse_Path_Engine_V4::get_instance()->get_viewing_context(); }
}
if (!function_exists('psych_complete_mission_by_flag_v4')) {
    function psych_complete_mission_by_flag_v4($flag_name, $user_id) {
...
final class PsychoCourse_Path_Engine_V4 {
    ...
    private function render_timeline_mode($stations) { /* ... */ return 'Timeline mode'; }
    ...
    public function ajax_get_station_content() {
        ...
        $station = json_decode(stripslashes($_POST['station_data'] ?? ''), true); if (!$station) wp_send_json_error();
    ...
    private function generate_mission_action_html($station) {
        switch ($station['mission_type']) {
            case 'gform': $form_id = intval(str_replace('form_id:', '', $station['mission_target']));
                if ($form_id > 0 && function_exists('gravity_form')) { $_SESSION['psych_path_gform_context'][$form_id] = ['station_node_id' => $station['station_node_id']]; return gravity_form($form_id, false, false, false, null, true, 0, false); }
                return '';
    ...
    public function handle_gform_submission($entry, $form) {
        ...
        if (isset($_SESSION['psych_path_gform_context'][$form['id']])) {
            ...
            $this->mark_station_as_completed($user_id, $context['station_node_id']); unset($_SESSION['psych_path_gform_context'][$form['id']]);
        }
    }
    public function ajax_complete_mission() {
        check_ajax_referer('psych_path_ajax_nonce', 'nonce'); $node_id = sanitize_key($_POST['node_id']);
    ...
    public function render_footer_elements() { if ($this->is_shortcode_active) { echo $this->get_path_styles_v4(); echo $this->get_path_javascript_v4(); } }
    private function get_path_styles_v4() {
        return '<style>
            :root { --psych-primary: #007bff; --psych-success: #28a745; --psych-locked: #6c757d; --psych-bg: #f8f9fa; }
            .psych-path-container { font-family: sans-serif; } .psych-path-header { text-align: center; margin-bottom: 20px; }
            .psych-progress-indicator { background: #e9ecef; border-radius: 8px; overflow: hidden; margin-bottom: 20px; }
            .psych-progress-bar { height: 10px; background: var(--psych-primary); transition: width 0.4s ease; }
            .psych-progress-text { display: block; text-align: center; font-size: 12px; padding: 2px; }
            .psych-station-item { border: 1px solid #dee2e6; margin-bottom: 10px; border-radius: 8px; transition: all 0.3s ease; }
            .psych-station-header { display: flex; align-items: center; padding: 15px; cursor: pointer; }
            .psych-station-icon { width: 30px; height: 30px; border-radius: 50%; color: white; display: inline-flex; align-items: center; justify-content: center; margin-right: 15px; background: var(--psych-locked); }
            .psych-station-item.open .psych-station-icon { background: var(--psych-primary); }
            .psych-station-item.completed .psych-station-icon { background: var(--psych-success); }
            .psych-station-title { font-weight: bold; flex-grow: 1; }
            .psych-station-status-badge { font-size: 12px; padding: 3px 8px; border-radius: 12px; color: white; }
            .psych-station-item.locked .psych-station-status-badge { background: var(--psych-locked); }
            .psych-station-item.open .psych-station-status-badge { background: var(--psych-primary); }
            .psych-station-item.completed .psych-station-status-badge { background: var(--psych-success); }
            .psych-accordion-toggle { transition: transform 0.3s; }
            .psych-station-item.content-open .psych-accordion-toggle { transform: rotate(180deg); }
            .psych-station-content { display: none; padding: 20px; border-top: 1px solid #dee2e6; }
            .psych-complete-mission-btn { background: var(--psych-primary); color: white; border: 0; padding: 10px 15px; border-radius: 5px; cursor: pointer; }
        </style>';
    }
    private function get_path_javascript_v4() { ob_start(); ?>
        <script>
        document.addEventListener('DOMContentLoaded', () => {
            window.psychPathApps = new Map();
            class PsychPathApp {
                ...
                toggleAccordion(id) {
                    ...
                    station.el.classList.remove('content-open');
                ...
                updateUI() {
                    this.stations.forEach(s => {
                        const { el, data } = s; el.className = `psych-station-item ${data.status}`;
            ...
            document.querySelectorAll('.psych-path-container').forEach(el => window.psychPathApps.set(el.id, new PsychPathApp(el)));
        });
        </script>
        <?php return ob_get_clean();
    }
}
PsychoCourse_Path_Engine_V4::get_instance();
